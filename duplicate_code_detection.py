...
def run(
    ...
    for source_file in source_code:
        sample_doc.append(tk.tokenize(docs) for docs in source_code[source_file])

    main_docs = list()
    for tokens in sample_doc:
        main_docs.append([token.replace(" ", "") for token in tokens])

    local_doc = [" ".join(one_doc) for one_doc in main_docs]
    main_docs = local_doc

    gen_docs = [
        [word.lower() for word in word_tokenize(main_docs[source_file])]
        for source_file in range(0, len(main_docs))
    ]
    ...
    for source_file in source_code:
        ...
        sample_doc = tk.tokenize(source_code[source_file])

        main_doc = list()
        for token in sample_doc:
            main_doc.append(token.replace(" ", ""))

        local_doc = " ".join(main_doc)
        main_doc = local_doc

        gen_docs = [
            [word.lower() for word in word_tokenize(main_doc)]
            for source_file in range(0, len(main_docs))
        ]
